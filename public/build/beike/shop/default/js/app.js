...
var _excluded = ["sku_id", "quantity", "isBuyNow"];
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  openWin: function openWin(url) {
    ...
    var iWidth = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 700;
    var iHeight = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 500;
  ...
  productImageResize: function productImageResize() {
    // 确保 config 存在并且配置正确
    if (!config || typeof config.productImageOriginWidth !== 'number' || typeof config.productImageOriginHeight !== 'number' || config.productImageOriginWidth <= 0 || config.productImageOriginHeight <= 0) {
    ...
    $productWrap.each(function () {
      var $images = $(this).find('.image-old');
      if ($images.length === 0) {
        return;
      }
      var productWrapWidth = $(this).width();
      if (typeof productWrapWidth !== 'number' || productWrapWidth <= 0) {
        return;
      }

      // 根据后台设置的图片尺寸比例计算列表商品图片高度
      var ratio = config.productImageOriginWidth / config.productImageOriginHeight;
      var height = productWrapWidth / ratio;
...
/***/ "./resources/beike/shop/default/js/footer.js"
/*!***************************************************!*\
  !*** ./resources/beike/shop/default/js/footer.js ***!
  \***************************************************/
() {
...
$(function () {
  ...
  $(document).on("click", ".btn-right-cart", function () {
    ...
    var offcanvasRightCart = new bootstrap.Offcanvas('#offcanvas-right-cart');
  ...
  $(document).on("click", "#offcanvas-right-cart .product-list .select-wrap", function () {
    var unchecked = 'bi bi-circle',
      checked = 'bi bi-check-circle-fill';
    var productListAll = $('#offcanvas-right-cart .product-list').length;
    var cartId = $(this).find('i.bi').data('id');
    var isChecked = $(this).children('i').hasClass(unchecked);
    $(this).children('i').prop('class', isChecked ? checked : unchecked);
    var checkedProduct = $('#offcanvas-right-cart .offcanvas-right-products i.bi-check-circle-fill').length;
    $('.offcanvas-footer .all-select i').prop('class', productListAll == checkedProduct ? checked : unchecked);
    var checkedIds = $('#offcanvas-right-cart .product-list').map(function () {
    ...
    $http.post("/carts/".concat(isChecked ? 'select' : 'unselect'), {
      cart_ids: [cartId]
    }, {
      hload: true
    }).then(function (res) {
  ...
  $('.menu-wrap > ul > li').each(function (index, el) {
    if ($(el).children('.dropdown-menu').length) {
      var offsetLeft = $(el).children('.dropdown-menu').offset().left;
      var width = $(el).children('.dropdown-menu').width();
      var offsetRight = offsetLeft + width;
      var windowWidth = $(window).width();
      if (offsetLeft < 0 || offsetRight > windowWidth) {
        $(el).addClass('position-static').children('.dropdown-menu').css({
  ...
  // PC/MB 头部滑动固定
  (function () {
    var pbHeader = $(window).width() > 768 ? $('.header-content') : $('.header-mobile');
    if (!pbHeader.length) return;
    var headerContentTop = pbHeader.offset().top;
    var headerContentHeight = pbHeader.outerHeight(true);
    $(window).scroll(function () {
      var scrollTop = $(this).scrollTop();
      if (scrollTop > headerContentTop) {
        pbHeader.addClass('fixed');
        if (!$('.header-content-placeholder').length) pbHeader.before('<div class="header-content-placeholder" style="height: ' + headerContentHeight + 'px"></div>');
      } else {
...
/* harmony import */ var _footer__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./footer */ "./resources/beike/shop/default/js/footer.js");
...
/* harmony import */ var _bootstrap_validation__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./bootstrap-validation */ "./resources/beike/shop/default/js/bootstrap-validation.js");
